import {norm, subdivision, randomColor, computeNormals, xyzmodel, lmsmodel} from "./util.js"
...
function cube_GetInfo(){
    let cube = [
        //Front 
        -myValue,  myValue, myValue,//V0 -> (-1, 1)
         myValue,  myValue, myValue,//V1 -> ( 1, 1) 
        -myValue, -myValue, myValue,//V2 -> (-1,-1) 
         myValue, -myValue, myValue,//V3 -> ( 1,-1) 
        //Back 
        -myValue,  myValue, -myValue,//V4 -> (-1, 1)
         myValue,  myValue, -myValue,//V5 -> ( 1, 1)
        -myValue, -myValue, -myValue,//V6 -> (-1,-1)
         myValue, -myValue, -myValue,//V7 -> ( 1,-1)
    ];
...
var icosahedron_GetInfo = function (){
    ...
    var subdivided = subdivision(0, vertexData, indicesData, radius)

    normalData = computeNormals(subdivided.vertices, subdivided.indices)

    let colorData = [];

    // for (let face=0; face<subdivided.indices.length; face++) {
    //     colorData.push(...[1,0,0]);
    // }

    for (let face=0; face<subdivided.indices.length/3; face++) {
        let faceColor= randomColor();
        faceColor = lmsmodel(faceColor);
...
function prism_GetInfo(){
    ...
    let normalData = computeNormals(prism, indicesPrism)
...
function pyramid_GetInfo(){
    ...
    let normalData = computeNormals(pyramid, indicesPyramid)
...
function prism1_GetInfo(){
    ...
    let normalData = computeNormals(prism1, indicesprism1)
...
function prism2_GetInfo(){
    ...
    let normalData = computeNormals(prism2, indicesprism2)
...
function prism3_GetInfo(){
    ...
    let normalData = computeNormals(prism3, indicesprism3)
...
function prism4_GetInfo(){
    ...
    let normalData = computeNormals(prism4, indicesprism4)
...
for (let face = 0; face < 8; face++){
    ...
    var normalData = computeNormals(prism, indicesPrism)
