...
function build_scene() {
	...
	// ground
	mat4.translate(scene[0].posRotMatrix, [-5, -1.0, -15.0]);
	...
	var cube_satellite = scene[0].children[3].children[1].children[1].children[1].children[0].children[1];
	
	cube_satellite.shape = null; // have to go deeper
	cube_satellite.children = [new Object3D(cube)];
	mat4.translate(cube_satellite.children[0].posRotMatrix, [0, -0.4, 0]);
	mat4.scale(cube_satellite.sclMatrix, [1.8, 1.8, 1.8]);
	...
	cube_satellite.update_function = spinnerx;
	cube_satellite.spinspeedfactor = 20;

	/////////////////////////////////////////////////////////////////////////////
	scene[1].update_function = spinnery;

	// sun
	mat4.translate(scene[1].posRotMatrix, [0, 0.0, 0.0]);

	// ground
	mat4.translate(scene[1].posRotMatrix, [5, -1.0, -15.0]);
	mat4.translate(scene[1].children[1].posRotMatrix, [0.0, -3.0, 0.0]);
	mat4.rotate(scene[1].children[1].posRotMatrix, degToRad(90), [1, 0, 0]); // x clockwise
	mat4.scale(scene[1].children[1].sclMatrix, [2, 2, 0.1]);

	// ground -> sun
	mat4.translate(scene[1].children[2].posRotMatrix, [0.0, 1.1-3, 0.0]);
	mat4.rotate(scene[1].children[2].posRotMatrix, degToRad(90), [1, 0, 0]); // x clockwise
	mat4.scale(scene[1].children[2].sclMatrix, [0.05, 0.05, 1]);

	// second planet rotation
	mat4.rotate(scene[1].children[3].posRotMatrix, degToRad(160), [0, 1, 0]); // x clockwise

	// moon1
	scene[1].children[3].children[1].children[0].update_function = spinnery;
	scene[1].children[3].children[1].children[0].spinspeedfactor = 5;

	// moon2
	mat4.rotate(scene[1].children[3].children[1].children[1].posRotMatrix, degToRad(160), [0, 1, 0]);
	scene[1].children[3].children[1].children[1].update_function = spinnery;
	scene[1].children[3].children[1].children[1].spinspeedfactor = 3;

	//  734p07 547311i73
	
	/*var cube_satellite = scene[1].children[3].children[1].children[1].children[1].children[0].children[1];
	...
	scene[1].children[3].children[1].children[1].children[1].update_function = spinnery;
	scene[1].children[3].children[1].children[1].children[1].spinspeedfactor = 5;
...
//	var vertexTextureBuffer=shape.textureBuffer;
